...
def SendEmail(mail_from, mail_to, mail_subject, message):
    ...
    msg_html = MIMEText(message, _subtype='html', _charset='utf-8')
    msg.attach(msg_html)
...
if __name__ == '__main__':
    if len(sys.argv) < 5:
        print 'usage: python ' + sys.argv[0] + ' from to subject message_file'
    else:
        SendEmail(sys.argv[1], sys.argv[2], sys.argv[3], open(sys.argv[4]).read())
