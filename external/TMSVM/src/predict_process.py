...
def ctm_predict(f_lib,config_file,indexes,result_save_path,result_indexes,str_splitTag,tc_splitTag,seg,delete=False,change_decode=False,in_decode="UTF-8",out_encode="GBK"):
    '''一般形式的下得模型预测，即单个模型。'''
    ...
    for line in lines:
        ...
        #y=[float(txt[0].strip())]
        y=[float(1)]
        while i in range(1,len(txt)):
            index=txt[i].strip().split(':')[0]
            value=txt[i].strip().split(':')[1]
            x[index]=value            
    '''
    
    label,sc=cal_sc_optim(model_file_name,data_file_name)
    fs.write(str(label)+"\t"+str(sc)+"\t")
        #for index in result_indexes:
          #  fs.write(text[index]+"\t")
    fs.write("\n")
